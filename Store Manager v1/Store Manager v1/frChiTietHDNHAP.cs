...
{
    public partial class frChiTietHDNHAP : Form
    {
        string nhanvien = "";
        string doitac = "";
        string mahd = "";
        List<ChitietHD> ct = new List<ChitietHD>();
        public frChiTietHDNHAP(string _mahd, string _nv, string _dt, List<ChitietHD> _ct)
        {
            mahd = _mahd;
            nhanvien = _nv;
            doitac = _dt;
        ...
        private void frChiTietHDNHAP_Load(object sender, EventArgs e)
        {
            lbhoadon.Text = mahd;
            lbnhanvien.Text = nhanvien;
            lbdoitac.Text = doitac;
            int tongtien = 0;
            for (int i = 0; i < ct.Count; i++)
            {
                ListViewItem item = new ListViewItem();
                item.Text = ct[i].mah;
                listnhap.Items.Add(item);
        ...
        private void _CreateReceipt(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            foreach (ListViewItem item in listnhap.Items)
            {
